...
const FacilityView = () => {
    ...
    async function apiCall() {
      try {
        let response = await axios.post(
          ...
          { facility_id: facilityData.facilityInfo.facility_id },
          {
            headers: {
  ...
  return (
    <section>
        ...
        <div>
          ...
          <div className="rounded-lg bg-gray-900 p-4 text-white">
            <ul className="space-y-2">
              {facilityView?.availableHours?.map((item, index) => (
                <li key={index} className="flex justify-between">
                  <span className="capitalize">{item.week_day}</span>
          ...
          <div className="mt-4">
            <p className="flex items-center">
              ...
              <span>
                Hourly Rate - ${facilityView?.facilityInfo?.hourly_rate}
              </span>
            </p>
            <p className="mt-2 flex items-center">
              ...
              <span>
                Capacity: {facilityView?.facilityInfo?.capacity} Person
              </span>
            </p>
          </div>
        </div>
      ...
      <Button className={"mt-5 w-full bg-darkSlate"} title={"Show More"} />
    </section>
  );
};

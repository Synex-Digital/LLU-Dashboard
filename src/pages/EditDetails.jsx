...
const EditDetails = () => {
  ...
  // Set your Map ID here
  const mapId = "your-map-id"; // Replace with your actual Map ID
  ...
  useEffect(() => {
    if (mapRef.current && location.lat && location.lng) {
      const loadMarker = async () => {
        try {
          // Dynamically import the AdvancedMarkerElement library
          const { AdvancedMarkerElement } = await google.maps.importLibrary("marker");

          if (markerRef.current) {
            // Update the position of the existing marker
            markerRef.current.setPosition(new google.maps.LatLng(location.lat, location.lng));
          } else {
  ...
  return (
    <section>
      ...
      <GoogleMap
        ...
        onLoad={onLoad}
        mapId={mapId} // Pass the Map ID here
      >
        {/* Advanced Marker will be used instead of the default Marker */}
      </GoogleMap>
      {loading ? (
        <Button
          title={<LoadingIcon />}

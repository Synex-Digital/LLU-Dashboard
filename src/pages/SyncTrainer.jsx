...
const SyncTrainer = () => {
  ...
  let handleEmployee = async (item) => {
    ...
    try {
      let response = await axios.post(
        `${baseUrl}/api/facilitator/add_employee`,
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        {trainers.map((trainer, index) => (
          ...
          >
            <div className="flex gap-x-3 items-center">
              <Image className={"w-24 rounded-lg"} src={profile} />
              <div>
                ...
                <p className="flex items-center gap-x-1 text-sm text-darkText">
                  ...
                  <span>
                    {trainer.avg_rating ? trainer.avg_rating.toFixed(1) : "N/A"}
                  </span>
                  <span>({trainer.no_of_reviews} Reviews)</span>
                </p>
              </div>
            </div>
            <Button

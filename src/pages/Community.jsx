...
const Community = () => {
  ...
  let handleComment = async (item) => {
    ...
    try {
      await axios.post(`${baseUrl}/api/user/comment`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
  ...
  let handleShowComment = async (item) => {
    ...
    try {
      let response = await axios.post(
        `${baseUrl}/api/user/individual_post?page=1&limit=10`,
        { post_id: item.post_id },
        {
          headers: {
            Authorization: `Bearer ${token}`,
                            ...
                            <div>
                              <div className="flex items-center gap-x-3">
                                <h3 className="text-lg capitalize cursor-pointer">
                                  {gitem.first_name} {gitem.last_name}
                                </h3>
                                <time className="text-darkText flex items-center">
